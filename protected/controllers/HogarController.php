...
class HogarController extends Controller
{
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['Hogar']))
		{
			...
			//$model->HOG_CLAVE=md5($model->HOG_CLAVE); //Codificar la clave de la tabla Hogar
			
			

			if($model->save()){
				yii::app()->authManager->assign("hogar",$model->HOG_N_USUARIO);
				$this->redirect(array('view','id'=>$model->HOG_N_USUARIO));
	...
	{
		$model=$this->loadModel($id);
		yii::app()->authManager->revoke("hogar",$model->HOG_N_USUARIO);			
		// Uncomment the following line if AJAX validation is needed
		// $this->performAjaxValidation($model);
